...
export const getters = {
  totalPrice: state => {
    ...
    return state.cart.reduce((initial, next) => initial + +next.totalPrice, 0);
  },
...
export const mutations = {
  updateFoodData: (state) => {
    state.fooddata = data
  },
  addToCart: (state, payload) => {
    // let itemExists = state.cart.some((cartItem) => cartItem.item == payload.item);
    let itemIndex = state.cart.findIndex((cartItem) => cartItem.item == payload.item);
    if (itemIndex >= 0) {
      state.cart[itemIndex] = payload;
      // state.cart[itemIndex].numberOfItems += Number(payload.numberOfItems);
    } else {
      payload.id = uuidv4();
...
export const actions = {
  async fetchFoodData({ state, commit }) {
    if (state.fooddata.length) return;
    try {
      await fetch('https://dva9vm8f1h.execute-api.us-east-2.amazonaws.com/production/restuarants', {
        headers: {
